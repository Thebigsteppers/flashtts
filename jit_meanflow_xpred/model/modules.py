...
class MelSpec(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert mel_spec_type in ["vocos", "bigvgan","hifigan"], print("We only support two extract mel backend: vocos or bigvgan")
...
def scaled_dot_product_attention_only(query, key, value, attn_mask=None, dropout_p=0.0,
    ...
    B = query.size(0)
    # H = query.size(1)  # number of heads
...
class BlockAttnProcessor:
    def __init__(self, block_size: int, t_p: int, t_f: int, pe_attn_head: int | None = None,):
        """
        Args:
            block_size (int): Number of tokens per block.
            t_p (int): Number of past blocks to attend to (0 = no past).
            t_f (int): Number of future blocks to attend to (0 = no future).
        """
        ...
        self.t_f = t_f
        self.mask = None
        self.pe_attn_head = None
        
    def try_cached_mask(self, seq_len, device):
        if self.mask is not None:
            if self.mask.size(0) == seq_len:
                return self.mask
        
        # Build block attention mask: block index per token, then diff matrix
        indices = torch.arange(seq_len, device=device)
        block_indices = indices // self.block_size

        diff = block_indices.unsqueeze(0) - block_indices.unsqueeze(1)  # [seq_len, seq_len]
        # Allow attention when diff in [-t_p, t_f]
        computed_mask = (diff >= -self.t_p) & (diff <= self.t_f)  # bool tensor, shape: [seq_len, seq_len]
        
        self.mask = computed_mask
        return self.mask
    ...
    def __call__(
        ...
        device = x.device
        # Project q, k, v and reshape to multi-head [batch, heads, seq_len, head_dim]
        query = attn.to_q(x)
        key = attn.to_k(x)
        value = attn.to_v(x)
...
class DiTBlock(nn.Module):
    def __init__(self, dim, heads, dim_head, ff_mult=4, dropout=0.1, block_size=24,t_p=0,t_f=0,qk_norm=None, pe_attn_head=None):
        super().__init__()

        self.attn_norm = AdaLayerNorm(dim)
        self.attn = Attention(
            processor=BlockAttnProcessor(block_size,t_p,t_f,pe_attn_head=pe_attn_head),
            dim=dim,
            heads=heads,
            dim_head=dim_head,
            dropout=dropout,
