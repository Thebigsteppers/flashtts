...
from third_party.campplus.tools import SpkEmbedding
...
class CosyVoiceFrontEnd:
    ...
    def __init__(self,
        ...
        self.spk_embedding = SpkEmbedding(device='cpu', half=False)
    ...
    def text_normalize(self, text, split=True, text_frontend=True):
        ...
        if self.use_ttsfrd:
            ...
            if contains_chinese(text):
                ...
                text = re.sub(r'[，,、]+$', '。', text)
                texts = list(split_paragraph(text, partial(self.tokenizer.encode, allowed_special=self.allowed_special), "zh", token_max_n=800,
    ...
    def frontend_zero_shot(self, tts_text, prompt_text, prompt_speech_16k, resample_rate, zero_shot_spk_id, is_from_speech, is_from_text, cfg_drop):
        tts_text_token, tts_text_token_len = self._extract_text_token(tts_text)
        if zero_shot_spk_id == '':
            spk_embedding = self.spk_embedding.extract_features([prompt_speech_16k.squeeze(dim=0)],[int(prompt_speech_16k.shape[1])])
            if cfg_drop > 0:
                spk_embedding_drop = torch.zeros(1, 192).to(self.device)
            else:
            ...
            model_input = {'prompt_text': prompt_text_token, 'prompt_text_len': prompt_text_token_len,
                           ...
                           'spk_embedding_drop': spk_embedding_drop,
                           'is_from_speech': is_from_speech,
